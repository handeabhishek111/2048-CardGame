...
function checkStatus() {
  ...
  const settings = {
    async: false,
    crossDomain: true,
    url: `/result/check/${user.get('ethAddress')}`,
    method: 'get',
    processData: false,
  };
  ...
  $.ajax(settings).done(async function (response) {
    ...
    if (response && response.status) {
      ...
      if (response.status == 'loser') {
        alert('Sorry you have lost the Duel');
      } else if (response.status == 'winner') {
        alert(
          'Congrats you have won the Duel, to claim the rewards you will have to burn your NFT'
        );
        await postWonChanges(user, response.tokenId);
      }
...
async function postWonChanges(user, tokenId) {
  //   burn NFT code
  const options721 = {
  ...
  const settings = {
    async: false,
    crossDomain: true,
    url: `/result/postWonChanges/${user.get('ethAddress')}`,
    method: 'get',
    processData: false,
  };
  $.ajax(settings).done(function (response) {
    console.log(response);
    if (response && response.status) {
      $('#duel-start-btn').css('display', 'none');
      $('#result-btn').css('display', 'block');
      if (response.status == 'winner') {
        alert('Now you can check your wallet for the transaction');
...
document.getElementById('btn-result').onclick = checkStatus;
